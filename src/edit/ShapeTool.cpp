...
void applyShapes(QImage &img, const QVector<ShapeOp> &shapes, const QPoint &cropOffset,
                  double scale) {
    ...
    for (const ShapeOp &op : shapes) {
        ...
        local.rect = QRectF((op.rect.topLeft() - QPointF(cropOffset)) * scale,
                             op.rect.size() * scale);
        local.p1 = (op.p1 - QPointF(cropOffset)) * scale;
        local.p2 = (op.p2 - QPointF(cropOffset)) * scale;
