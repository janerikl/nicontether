...
int main(int argc, char **argv) {
    ...
    // Three plain layers. addMask inserts each new layer at index 0 (top of
    // the stack) and selects it, so after three calls masks() is
    // [L2, L1, L0] (insertion order, most-recent first) with L2 active.
    RetouchTab tab(QSize(8, 8));
    tab.addMask(MaskType::Radial);
    tab.addMask(MaskType::Radial);
    tab.addMask(MaskType::Radial);
    assert(tab.masks().size() == 3);
    ...
    {
        tab.reorderMasks({2, 0, 1});
        assert(tab.masks().size() == 3);
        assert(tab.activeMaskIndex() == 1);
    }

    // Reset to a known order for the next cases: identity [0, 1, 2].
    tab.reorderMasks({1, 2, 0}); // undoes the previous permutation
    tab.selectMask(1);
    assert(tab.activeMaskIndex() == 1);
    ...
    // outright rather than silently dropping/duplicating a mask.
    {
        ...
        tab.reorderMasks({0, 0, 2}); // index 1 missing, index 0 duplicated
    ...
    // layer out of its group) while leaving the group's other member tagged.
    {
        ...
        tab.reorderMasks({0, 1, 2}, {0}); // identity order, but layer 0 left its group
        ...
        tab.reorderMasks({0, 1, 2}, {}, {{0, groupId}});
